...
export class ShortTermMemoryImpl implements ShortTermMemoryService {
  ...
  getStats(): {
    ...
    return {
      ...
      totalTokens: this.messages.reduce((sum, m) => sum + (m.tokenCount ?? 0), 0),
  ...
  mergeConsecutiveUserMessages(maxAgeMs?: number): number {
    ...
    for (let i = 0; i < this.messages.length; i++) {
      ...
      while (i + 1 < this.messages.length && this.messages[i + 1].role === 'user') {
      ...
      if (group.length > 1) {
        ...
        const combinedTokens = group.reduce((s, m) => s + (m.tokenCount ?? 0), 0);
        result.push({
          role: 'user',
