...
import type { MemoryManager } from './types.js';
...
export class MemorySecurityIntegration {
  ...
  extractSecureEntities(text: string): Array<{
    ...
    const orgPattern = /\b(Google|Apple|Microsoft|Amazon|Meta|OpenAI|Anthropic|Tesla)\b/gi;
    for (const match of sanitizeResult.sanitized.matchAll(orgPattern)) {
      entities.push({ type: 'Organization', name: match[0] });
    }

    // Location patterns
    const locPattern = /\b(San Francisco|New York|London|Paris|Tokyo|Beijing)\b/gi;
