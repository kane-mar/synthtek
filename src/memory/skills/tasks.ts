...
export class MemoryTasksSkill implements MemorySkill {
   ...
   */
  async updateStatus(id: string, status: TaskStatus): Promise<TaskEntry | null> {
   ...
   */
  async updatePriority(id: string, priority: number): Promise<TaskEntry | null> {
    const task = this.tasks.get(id);
    if (!task) return null;
    ...
    for (const entry of result.entries) {
      ...
      if (taskId) {
        ...
        this.tasks.set(taskId, {
          ...
          status,
          priority: 3,
          tags: (entry.metadata.tags as string[])?.filter((t: string) => t !== 'task') ?? [],
