...
import type { IngestResult, MemoryIngestConfig, MemorySkill } from "./types.js";
...
export class MemoryIngestSkill implements MemorySkill {
	...
	async ingestText(
		...
		for (
			let i = 0;
			i < chunks.length && entriesCreated < this.config.maxEntries;
			i++
		) {
			try {
				...
				errors.push(
					`Failed to ingest chunk ${i}: ${err instanceof Error ? err.message : String(err)}`,
				);
			}
		}
	...
	async ingestJSON(
		...
		for (
			let i = 0;
			i < data.length && entriesCreated < this.config.maxEntries;
			i++
		) {
			try {
				const item = data[i];
				...
				errors.push(
					`Failed to ingest item ${i}: ${err instanceof Error ? err.message : String(err)}`,
				);
			}
		}
