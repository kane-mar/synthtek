...
const DEFAULT_CONFIG: Required<Omit<EncryptionConfig, 'useKeyFile' | 'keyFilePath'>> = {
...
export class ApiKeyEncryption {
  ...
  async encrypt(plaintext: string): Promise<EncryptionResult> {
    try {
      ...
      const ivLength = this.config.algorithm === 'aes-256-gcm' ? IV_LENGTH_GCM : IV_LENGTH_CBC;
  ...
  async decrypt(encryptedValue: string): Promise<EncryptionResult> {
    try {
      ...
      if (!payload.ciphertext || !payload.iv || !payload.salt) {
        return { success: false, error: 'Missing required fields in encrypted payload' };
      }
      ...
      const decrypted = this.decryptWithKey(derivedKey, iv, payload.ciphertext, payload.tag);
