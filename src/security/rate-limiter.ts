...
export class RateLimiter {
  ...
  check(userId: string): RateLimitResult {
    ...
    if (userState.requests.length >= maxRequests) {
      if (this._config.banOnExceed) {
        ...
        userState.banExpiry = Date.now() + (this._config.banDurationMs ?? 300000);
      }
      return {
        allowed: false,
        remaining: 0,
  ...
  stats(userId: string): { totalRequests: number; windowRequests: number; banned: boolean } | null {
    const userState = this._getUserState(userId);
    const windowMs = this._getWindowMs(userId);
    const cutoff = Date.now() - windowMs;
    ...
    return {
      ...
      windowRequests: userState.requests.filter((r) => r.timestamp > cutoff).length,
      banned: userState.banned,
    };
  }
