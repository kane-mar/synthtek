...
export class ShellSandbox {
	...
	validate(command: string): SandboxResult {
		...
		if (
			this._config.allowedCommands &&
			!this._config.allowedCommands.includes(cmd)
		) {
			return {
				allowed: false,
				reason: `Command '${cmd}' is not in the allowed list`,
			};
		}
		...
		if (this._config.maxArgs && args.length > this._config.maxArgs) {
			return {
				allowed: false,
				reason: `Too many arguments (max ${this._config.maxArgs})`,
			};
		}
	...
	private _parseCommand(
		command: string,
	): { cmd: string; args: string[]; paths: string[] } | null {
	...
	private _isPathAllowed(path: string): boolean {
		...
		return this._config.allowedPathPrefixes.some((prefix) =>
