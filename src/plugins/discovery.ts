	...
	private extractManifest(
		...
		return {
			...
			description:
				...
				(synthtek?.description as string | undefined),
			author:
				(pkg.author as string) || (synthtek?.author as string | undefined),
			requires: (synthtek?.requires as string[]) || undefined,
			configSchema:
				(synthtek?.configSchema as Record<string, unknown>) || undefined,
