  ...
  async init(plugins: Plugin[]): Promise<void> {
    ...
    for (const plugin of plugins) {
      try {
        ...
        this.globalEvents.emit('plugin:initialized', { name: plugin.manifest.name });
      } catch (error) {
        this.state.set(plugin.manifest.name, {
          name: plugin.manifest.name,
          status: 'error',
        ...
        this.globalLogger.error(`Plugin "${plugin.manifest.name}" init failed:`, { error: String(error) });
  ...
  async run(plugins: Plugin[]): Promise<void> {
    ...
    const promises = plugins.map(async (plugin) => {
      ...
      try {
        ...
        this.globalEvents.emit('plugin:started', { name: plugin.manifest.name });
      } catch (error) {
        this.state.set(plugin.manifest.name, {
          name: plugin.manifest.name,
          status: 'error',
        ...
        this.globalLogger.error(`Plugin "${plugin.manifest.name}" start failed:`, { error: String(error) });
        this.globalEvents.emit('plugin:error', {
          name: plugin.manifest.name,
          error: error instanceof Error ? error.message : String(error),
        });
  ...
  async teardown(plugins: Plugin[]): Promise<void> {
    ...
    for (const plugin of reversed) {
      try {
        ...
        this.globalEvents.emit('plugin:teardown', { name: plugin.manifest.name });
      } catch (error) {
