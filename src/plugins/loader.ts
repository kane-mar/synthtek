	...
	async load(discovered: DiscoveredPlugin[]): Promise<Plugin[]> {
		...
		for (const plugin of ordered) {
			try {
				...
				this.globalLogger.error(`Failed to load plugin "${plugin.name}":`, {
					error: String(error),
				});
			}
		}
	...
	private async loadPlugin(discovered: DiscoveredPlugin): Promise<Plugin> {
		...
		if (this.config.validateConfig && discovered.manifest.configSchema) {
			this.validateConfig(
				...
				discovered.manifest.configSchema,
			);
		}
		const context: PluginContext = {
			...
			emit: (event: string, data: unknown) =>
				this.globalEvents.emit(event, data),
		...
		try {
			...
			if (typeof plugin !== "object" || typeof plugin.init !== "function") {
				throw new Error(
					`Plugin "${discovered.name}" does not export a valid plugin object.`,
				);
			}
	...
	private createPluginLogger(pluginName: string): Logger {
		...
		return {
			debug: (msg: string, meta?: Record<string, unknown>) =>
				this.globalLogger.debug(`${prefix} ${msg}`, {
					...meta,
					plugin: pluginName,
				}),
			info: (msg: string, meta?: Record<string, unknown>) =>
				this.globalLogger.info(`${prefix} ${msg}`, {
					...meta,
					plugin: pluginName,
				}),
			warn: (msg: string, meta?: Record<string, unknown>) =>
				this.globalLogger.warn(`${prefix} ${msg}`, {
					...meta,
					plugin: pluginName,
				}),
			error: (msg: string, meta?: Record<string, unknown>) =>
				this.globalLogger.error(`${prefix} ${msg}`, {
					...meta,
					plugin: pluginName,
				}),
		};
