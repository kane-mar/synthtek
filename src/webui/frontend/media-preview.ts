...
export class MediaPreviewComponent {
  ...
  filterByType(type: 'image' | 'audio' | 'video' | 'document'): MediaPreviewInfo[] {
    return this.mediaItems.filter((item) => item.mimeType.startsWith(`${type}/`));
  }

  filterBySize(min: number, max: number): MediaPreviewInfo[] {
    return this.mediaItems.filter((item) => item.size >= min && item.size <= max);
  }
