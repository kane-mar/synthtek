...
const MOCK_VERSIONS: Record<string, SkillVersion[]> = {
  'web-search': [
    { version: '1.2.0', releaseDate: '2026-04-01', changelog: 'Improved search accuracy', isLatest: true },
    { version: '1.1.0', releaseDate: '2026-03-15', changelog: 'Added filters', isLatest: false },
    { version: '1.0.0', releaseDate: '2026-02-01', changelog: 'Initial release', isLatest: false },
  ],
  'memory-manager': [
    { version: '2.0.0', releaseDate: '2026-04-10', changelog: 'Major refactor', isLatest: true },
    { version: '1.0.0', releaseDate: '2026-01-01', changelog: 'Initial release', isLatest: false },
  ],
};
...
export class ClawHubClient {
  ...
  searchSkills(query: string, filters?: SearchFilters): SkillSearchResult[] {
    try {
      let results = MOCK_SKILLS.filter((s) =>
  ...
  updateSkill(name: string, _fromVersion: string, toVersion: string): SkillInstallResult | null {
  ...
  recordDiscoveredSkill(skill: { name: string; category: string; tags: string[] }): DreamLearnedSkill {
