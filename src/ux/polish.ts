...
const MODEL_DATABASE: Record<string, ModelSuggestion[]> = {
  openai: [
    { name: 'gpt-4o', provider: 'openai', description: 'Latest flagship model', recommended: true },
    { name: 'gpt-4o-mini', provider: 'openai', description: 'Fast and efficient', recommended: false },
    { name: 'gpt-4-turbo', provider: 'openai', description: 'Turbo variant', recommended: false },
    { name: 'gpt-3.5-turbo', provider: 'openai', description: 'Cost-effective', recommended: false },
    { name: 'o1', provider: 'openai', description: 'Reasoning model', recommended: false },
    { name: 'o3', provider: 'openai', description: 'Advanced reasoning', recommended: false },
  ],
  anthropic: [
    { name: 'claude-sonnet-4-20250514', provider: 'anthropic', description: 'Latest Sonnet', recommended: true },
    { name: 'claude-opus-4-20250514', provider: 'anthropic', description: 'Most capable', recommended: false },
    { name: 'claude-haiku-3-5-20241022', provider: 'anthropic', description: 'Fast and efficient', recommended: false },
  ],
  ollama: [
    { name: 'llama3.1', provider: 'ollama', description: 'Meta Llama 3.1', recommended: true },
    { name: 'mistral', provider: 'ollama', description: 'Mistral 7B', recommended: false },
    { name: 'codellama', provider: 'ollama', description: 'Code-focused', recommended: false },
    { name: 'phi3', provider: 'ollama', description: 'Microsoft Phi-3', recommended: false },
  ],
};
...
const ERROR_SUGGESTIONS: Record<string, { message: string; suggestion: string }> = {
  api_key_missing: {
    message: 'API key is not configured.',
    suggestion: 'Run `synthtek config set api_key <your-key>` or set the SYNTHTEK_API_KEY environment variable.',
  },
  connection_failed: {
    message: 'Failed to connect to the provider.',
    suggestion: 'Check your network connection and verify the provider URL is correct.',
  },
  rate_limited: {
    message: 'Rate limit exceeded.',
    suggestion: 'Wait a moment before retrying, or increase your rate limit in the provider settings.',
  },
  invalid_model: {
...
export class UXPolish {
  ...
  getWizardSteps(): SetupWizardStep[] {
    return [
      ...
      },
      {
        ...
        description: 'Choose your AI provider (OpenAI, Anthropic, Ollama, etc.)',
        required: true,
      },
      {
        id: 'model',
      ...
      },
      {
        ...
        description: 'Choose how you want to interact (CLI, Telegram, Discord, etc.)',
        required: false,
      },
      {
        id: 'complete',
  ...
  createProgressUpdate(task: string, current: number, total: number): ProgressUpdate {
    return {
      task,
      current,
      total,
  ...
  generateProgressBar(current: number, total: number, width: number = 20): string {
