...
import { FileSystemService } from '../core/types.js';
import { ExecutorService } from '../core/types.js';
import { SearcherService } from '../core/types.js';
...
function registerSearchTools(
  ...
  server.registerTool({
    ...
    handler: async (args: Record<string, unknown>) => {
      ...
      const headLimit = args.headLimit
        ? Number(args.headLimit)
        : undefined;
  ...
  server.registerTool({
    ...
    parameters: {
      ...
      properties: {
        ...
        glob: { type: 'string', description: 'File filter pattern (e.g. *.py)' },
        outputMode: {
          type: 'string',
          description:
