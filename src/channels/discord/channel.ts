...
function parseMessage(msg: any): DiscordMessage {
  return {
    ...
    fromDiscriminator: msg.author.discriminator !== '0' ? msg.author.discriminator : undefined,
    text: msg.content,
    ...
    embeds: msg.embeds?.map((e: any) => ({
      ...
      author: e.author ? { name: e.author.name, iconURL: e.author.iconURL ?? undefined } : undefined,
    })) ?? [],
...
function buildEmbed(embed: DiscordEmbed): EmbedBuilder {
  ...
  if (embed.author) builder.setAuthor({ name: embed.author.name, iconURL: embed.author.iconUrl });
...
export class DiscordChannel {
  ...
  private setupEventHandlers(): void {
    ...
    this.client.on('messageCreate', (msg: any) => {
      ...
      if (this.config.guildIds.length > 0 && msg.guildId && !this.config.guildIds.includes(msg.guildId)) {
        return;
      }
    ...
    this.client.on('messageUpdate', (_oldMsg: any, newMsg: any) => {
      ...
      if (this.config.guildIds.length > 0 && newMsg.guildId && !this.config.guildIds.includes(newMsg.guildId)) {
        return;
      }
    ...
    this.client.on('disconnect', () => {
      if (this.config.maxReconnectAttempts === 0 || this.reconnectAttempts < this.config.maxReconnectAttempts) {
        ...
        console.log(`[Discord] Disconnected. Reconnecting in ${this.config.reconnectDelay}ms (attempt ${this.reconnectAttempts})`);
        setTimeout(() => {
          if (this.started) {
  ...
  private isAdminCommand(command: string): boolean {
    const adminCommands = ['stop', 'restart', 'status', 'dream', 'dream_log', 'dream_restore'];
  ...
  private async registerSlashCommands(): Promise<void> {
    try {
      const commands: DiscordSlashCommand[] = [
        ...
        { name: 'dream_log', description: 'Show the latest Dream memory change' },
        { name: 'dream_restore', description: 'Restore Dream memory to an earlier version' },
  ...
  async start(onMessage: (message: DiscordMessage) => void, onError?: (error: Error) => void): Promise<void> {
  ...
  async sendMessage(
    ...
    if (options?.mentionAuthor) {
      sendOptions.allowedMentions = { parse: ['users'], users: [this.botUser.id] };
    }
    ...
    if (options?.embed) {
      ...
      return (ch as any).send({
        ...
        files: options?.files?.map((f) => new AttachmentBuilder(f.data, { name: f.name })),
  ...
  async sendEmbed(channelId: string, embed: DiscordEmbed): Promise<{ messageId: string }> {
    const ch = await this.getChannel(channelId);
    if (!ch) throw new Error(`Channel ${channelId} not found`);
  ...
  async sendWithButtons(
    ...
    const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
      buttons.map((b) => new ButtonBuilder().setLabel(b.label).setStyle(b.style).setCustomId(b.customId)),
    );
  ...
  async editMessage(channelId: string, messageId: string, content: string, options?: DiscordSendOptions): Promise<void> {
    const ch = await this.getChannel(channelId);
    if (!ch) throw new Error(`Channel ${channelId} not found`);
  ...
  async getReactions(channelId: string, messageId: string): Promise<DiscordReaction[]> {
    const ch = await this.getChannel(channelId);
    if (!ch) throw new Error(`Channel ${channelId} not found`);
  ...
  async getChannel(channelId: string): Promise<TextChannel | NewsChannel | VoiceChannel | StageChannel | ForumChannel | MediaChannel | ThreadChannel | DMChannel | null> {
    try {
  ...
  async getChannelInfo(channelId: string): Promise<DiscordChannelInfo | null> {
    ...
    if ('guildId' in channel && (channel as any).guildId) info.guildId = (channel as any).guildId;
  ...
  async hasPermission(channelId: string, permission: keyof DiscordPermission): Promise<boolean> {
    const channel = await this.getChannel(channelId);
  ...
  async getPermissions(channelId: string): Promise<DiscordPermission> {
    ...
    return {
      ...
      viewCreatorMonetizationAnalytics: perms.has(flags.ViewCreatorMonetizationAnalytics),
  ...
  async deleteCommand(commandName: string, guildId?: string): Promise<void> {
    ...
    if (guildId) {
      await rest.delete(
        Routes.applicationGuildCommand(this.config.clientId || '', guildId, commandName),
      );
    } else {
      await rest.delete(
  ...
  private defaultPermissions(): DiscordPermission {
    return Object.fromEntries(
      Object.keys(PermissionsBitField.Flags).map((k) => [k.toLowerCase(), false]),
