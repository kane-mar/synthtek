...
export class FeishuChannel extends BaseChannel<FeishuConfig, FeishuMessage> {
	...
	private getApiBase(): string {
		return this.getConfig().useGlobalDomain
			? DEFAULT_API_BASE_GLOBAL
			: DEFAULT_API_BASE_CN;
	}
	...
	private async authenticate(): Promise<void> {
		...
		if ((data.code as number) !== 0) {
			throw new Error(
				`Auth failed: ${(data.msg as string) ?? "Unknown error"}`,
			);
		}
	...
	async sendMessage(options: FeishuSendOptions): Promise<string> {
		...
		if ((data.code as number) !== 0) {
			throw new Error(
				`Send failed: ${(data.msg as string) ?? "Unknown error"}`,
			);
		}
	...
	async sendCard(options: FeishuCardOptions): Promise<FeishuCardResult> {
		...
		try {
			...
			if ((data.code as number) !== 0) {
				return {
					success: false,
					error: (data.msg as string) ?? "Unknown error",
				};
			}
	...
	async getUserInfo(userId: string): Promise<FeishuUserInfo> {
		...
		const data = response as Record<string, unknown>;
		const user =
			((data.data as Record<string, unknown>)?.user as Record<
				string,
				unknown
			>) ?? {};
		...
		return {
			...
			avatarUrl:
				((user.avatar as Record<string, unknown>)?.avatar_72 as string) ??
				undefined,
	...
	async getChatInfo(chatId: string): Promise<FeishuChatInfo> {
		...
		const data = response as Record<string, unknown>;
		const chat =
			((data.data as Record<string, unknown>)?.chat as Record<
				string,
				unknown
			>) ?? {};
				...
				try {
					...
					if (header?.event_type === "im.message.receive_v1" && event) {
						...
						const msg: FeishuMessage = {
							...
							chatType:
								(message.chat_type as FeishuMessage["chatType"]) ?? "p2p",
							senderId:
							...
							messageType:
								(message.message_type as FeishuMessage["messageType"]) ??
								"text",
							timestamp: Date.now(),
