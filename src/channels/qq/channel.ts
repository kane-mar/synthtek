...
export class QQChannel extends BaseChannel<QQConfig, QQMessage> {
  ...
  async connect(): Promise<void> {
    try {
      ...
      throw new Error(`QQ connection failed: ${err instanceof Error ? err.message : 'Unknown error'}`);
    }
  }
  ...
  private async authenticate(): Promise<void> {
    ...
    if ((data.retcode as number) !== 0 || !data.access_token) {
      throw new Error(`QQ auth failed: ${(data.message as string) ?? 'Invalid credentials'}`);
    }

    this.accessToken = data.access_token as string;
    this.tokenExpiry = Date.now() + ((data.expires_in as number) ?? 14400) * 1000;
  }
