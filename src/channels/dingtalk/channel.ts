...
export class DingTalkChannel extends BaseChannel<DingTalkConfig, DingTalkMessage> {
  ...
  private async authenticate(): Promise<void> {
    ...
    if ((data.errcode as number) !== 0) {
      throw new Error(`Auth failed: ${(data.errmsg as string) ?? 'Unknown error'}`);
    }
  ...
  async sendMessage(options: DingTalkSendOptions): Promise<DingTalkSendResult> {
    if (!this.isConnected()) {
      return { success: false, errorCode: -1, errorMessage: 'DingTalk channel is not connected' };
    }
    ...
    try {
      const response = await this.request('POST', '/v1.0/robot/oToMessages/batchSend', body);
      const data = response as Record<string, unknown>;
