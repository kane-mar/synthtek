...
export class WeComChannel extends BaseChannel<WeComConfig, WeComMessage> {
  ...
  async sendMessage(options: WeComSendOptions): Promise<WeComSendResult> {
    if (!this.isConnected()) {
      return { success: false, errorCode: -1, errorMessage: 'WeCom channel is not connected' };
    }
    ...
    const config = this.getConfig();
    const userIds = Array.isArray(options.userIds) ? options.userIds.join('|') : options.userIds;
    ...
    try {
      const response = await this.request('POST', '/cgi-bin/message/send', body);
      const data = response as Record<string, unknown>;
