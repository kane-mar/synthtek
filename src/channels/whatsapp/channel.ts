	...
	async sendMessage(options: WhatsAppSendOptions): Promise<WhatsAppSendResult> {
		...
		try {
			...
			if (options.type === "text") {
				...
				["image", "audio", "video", "document"].includes(options.type)
			) {
				body[options.type] = {
					id: options.mediaId,
					...
					const whatsappMsg: WhatsAppMessage = {
						...
						to:
							((value.metadata as Record<string, unknown>)
								?.display_phone_number as string) ?? "",
						timestamp: Date.now(),
						...
						media:
							msg.type !== "text"
								? {
										id:
											((
												msg[msg.type as keyof typeof msg] as Record<
													string,
													unknown
												>
											)?.id as string) ?? "",
						...
						reaction:
							msg.type === "reaction"
								? {
										message_id:
											((msg.reaction as Record<string, unknown>)
						...
						context: msg.context
							? {
									message_id:
										((msg.context as Record<string, unknown>)?.id as string) ??
										"",
									from:
