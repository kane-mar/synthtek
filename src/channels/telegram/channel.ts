...
function markdownToTelegramHtml(text: string): string {
  ...
  for (let idx = 0; idx < codeBlocks.length; idx++) {
    ...
    result = result.replace(`\x00CB${idx}\x00`, `<pre><code>${escaped}</code></pre>`);
  }
...
function parseMessage(update: any): TelegramMessage | null {
  ...
  return {
    ...
    messageType,
    caption: msg.caption,
    fileId: msg.photo?.[msg.photo.length - 1]?.file_id || msg.document?.file_id || msg.video?.file_id,
...
function splitMessage(text: string, maxLength = TELEGRAM_MAX_MESSAGE_LEN): string[] {
...
function mentionsBot(text: string | undefined, botUsername: string | undefined): boolean {
...
export class TelegramChannel {
  ...
  private typingTasks = new Map<number | string, ReturnType<typeof setInterval>>();
  private mediaGroupBuffers = new Map<number | string, TelegramMediaGroupBuffer>();
  private mediaGroupTimeouts = new Map<number | string, ReturnType<typeof setTimeout>>();
  ...
  /** Start long polling */
  async start(onMessage: (message: TelegramMessage) => void, onError?: (error: Error) => void): Promise<void> {
    ...
    try {
      ...
      await this.apiCallWithRetry('sendMessage', {
        ...
        reply_parameters: replyParams ? {
          ...
          allow_sending_without_reply: replyParams.allowSendingWithoutReply ?? true,
        } : undefined,
        ...threadKwargs,
      });
      this.messagesSent++;
  ...
  async sendOutboundMessage(msg: TelegramOutboundMessage): Promise<void> {
    ...
    const threadKwargs = messageThreadId !== undefined ? { message_thread_id: messageThreadId } : {};

    const replyParams = this.config.replyToMessage && replyToMessageId
    ...
    if (msg.content && msg.content !== '[empty message]') {
      ...
      for (const chunk of splitMessage(msg.content, this.config.maxMessageLength)) {
        await this.sendTextWithHtml(chatId, chunk, replyParams, threadKwargs, renderAsBlockquote);
      }
    }
  }
  ...
  async sendAlbum(
    chatId: number | string,
    media: Array<{ type: 'photo' | 'document'; media: string; caption?: string }>,
    options?: TelegramSendOptions,
    ...
    const data = await response.json() as { ok: boolean; result?: Array<{ message_id: number }> };
  ...
  /** Pin a message */
  async pinMessage(chatId: number | string, messageId: number, disableNotification?: boolean): Promise<boolean> {
  ...
  /** Unpin a message */
  async unpinMessage(chatId: number | string, messageId?: number): Promise<boolean> {
  ...
  async unpinAllMessages(chatId: number | string): Promise<boolean> {
    const response = await this.apiCall('unpinAllChatMessages', { chat_id: chatId });
    const data = await response.json() as { ok: boolean };
    return data.ok;
  }
  ...
  startStream(chatId: number | string, messageId: number, streamId: string): void {
  ...
  async addReaction(chatId: number | string, messageId: number, emoji: string): Promise<boolean> {
  ...
  async removeReactions(chatId: number | string, messageId: number): Promise<boolean> {
    const response = await this.apiCall('setMessageReaction', {
      chat_id: chatId,
      message_id: messageId,
  ...
  async getReactions(chatId: number | string, messageId: number): Promise<TelegramReaction[]> {
  ...
  /** Get chat info */
  async getChatInfo(chatId: number | string): Promise<TelegramChannelInfo | null> {
    ...
    return {
      ...
      photo: chat.photo ? { smallFileId: chat.photo.small_file_id, bigFileId: chat.photo.big_file_id } : undefined,
      bio: chat.bio,
  ...
  async getChatAdministrators(chatId: number | string): Promise<any[]> {
    const response = await this.apiCall('getChatAdministrators', { chat_id: chatId });
    const data = await response.json() as { ok: boolean; result?: any[] };
  ...
  async getChatMemberCount(chatId: number | string): Promise<number> {
    const response = await this.apiCall('getChatMembersCount', { chat_id: chatId });
  ...
  async getChatMember(chatId: number | string, userId: number): Promise<any | null> {
    const response = await this.apiCall('getChatMember', { chat_id: chatId, user_id: userId });
    const data = await response.json() as { ok: boolean; result?: any };
  ...
  async unbanChatMember(chatId: number | string, userId: number, onlyIfBanned?: boolean): Promise<boolean> {
    const body: Record<string, unknown> = { chat_id: chatId, user_id: userId };
  ...
  async getUserProfile(userId: number): Promise<TelegramUserInfo | null> {
    const response = await this.apiCall('getUserProfilePhotos', { user_id: userId, offset: 0, limit: 1 });
    const data = await response.json() as { ok: boolean; result?: { total_count: number; photos: any[][] } };

    if (!data.ok) return null;

    // Also get user info via getChat
    const chatResponse = await this.apiCall('getChat', { chat_id: userId });
    const chatData = await chatResponse.json() as { ok: boolean; result?: any };
  ...
  /** Set up webhook */
  async setWebhook(url: string, secretToken?: string, ipAddress?: string, maxConnections?: number, allowedUpdatesStr?: string): Promise<boolean> {
  ...
  /** Get webhook info */
  async getWebhookInfo(): Promise<{ url: string; hasCustomCertificate: boolean; pendingUpdateCount: number; lastErrorDate?: number; errorMessage?: string } | null> {
  ...
  async copyMessage(
    ...
    if (options?.disablePreviewLinks !== undefined) body.disable_web_page_preview = options.disablePreviewLinks;
    ...
    while (this.polling && this.started) {
      try {
        ...
        for (const update of updates) {
          ...
          if (!this.shouldProcessMessage(message.text, chatInfo?.type ?? 'private')) continue;
          ...
          if (this.config.reactEmoji) {
            try {
              await this.addReaction(message.chatId, message.messageId, this.config.reactEmoji);
            } catch {
        ...
        console.error(`[Telegram] Polling error (attempt ${this.reconnectAttempts}):`, error);
        ...
        const delay = Math.min(this.config.retryDelay * Math.pow(1.5, this.reconnectAttempts), 60000);
        await sleep(delay);
      }
    }
  }
  ...
  private async apiCallWithRetry(method: string, body?: Record<string, unknown>): Promise<Response> {
    for (let attempt = 1; attempt <= SEND_MAX_RETRIES; attempt++) {
      try {
        ...
        const isRateLimited = error.message?.includes('429') || error.message?.includes('flood');
        if (isRateLimited && attempt < SEND_MAX_RETRIES) {
          const delay = SEND_RETRY_BASE_DELAY_MS * Math.pow(2, attempt - 1);
          console.warn(`[Telegram] Rate limited (attempt ${attempt}/${SEND_MAX_RETRIES}), retrying in ${delay}ms`);
          await sleep(delay);
          continue;
        }
        // Check for timeout
        ...
        if (isTimeout && attempt < SEND_MAX_RETRIES) {
          const delay = SEND_RETRY_BASE_DELAY_MS * Math.pow(2, attempt - 1);
          console.warn(`[Telegram] Timeout (attempt ${attempt}/${SEND_MAX_RETRIES}), retrying in ${delay}ms`);
          await sleep(delay);
          continue;
        }
        throw error;
  ...
  private async apiCall(method: string, body?: Record<string, unknown>): Promise<Response> {
