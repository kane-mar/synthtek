...
export class EmailChannel extends BaseChannel<EmailConfig, EmailMessage> {
  ...
  async sendEmail(options: EmailSendOptions): Promise<EmailSendResult> {
    ...
    try {
      ...
      body += `From: ${config.fromAddress}\r\n`;
      body += `To: ${toList.join(', ')}\r\n`;
      if (ccList.length) body += `Cc: ${ccList.join(', ')}\r\n`;
      body += `Subject: ${this.encodeSubject(options.subject)}\r\n`;
      body += `Message-ID: ${messageId}\r\n`;
      body += `Date: ${new Date().toUTCString()}\r\n`;
      if (options.replyTo) body += `Reply-To: ${options.replyTo}\r\n`;

      if (options.priority) {
        body += `X-Priority: ${options.priority === 'high' ? '1' : options.priority === 'low' ? '5' : '3'}\r\n`;
        body += `Importance: ${options.priority === 'high' ? 'urgent' : options.priority === 'low' ? 'non-urgent' : 'normal'}\r\n`;
      }
      ...
      if (hasAttachments || hasBothTextAndHtml) {
        ...
        body += `Content-Type: multipart/mixed; boundary="${boundaries[0]}"\r\n`;
      }
  ...
  async processEmail(rawEmail: EmailMessage): Promise<void> {
    ...
    try {
      ...
      for (const att of rawEmail.attachments) {
        if (att.size <= (config.maxAttachmentSize ?? DEFAULT_MAX_ATTACHMENT_SIZE)) {
