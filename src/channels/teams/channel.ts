...
export class TeamsChannel extends BaseChannel<TeamsConfig, TeamsMessage> {
  ...
  async getConversationInfo(conversationId: string): Promise<TeamsConversationInfo> {
    if (!this.isConnected() || !this.authenticated) {
      throw new Error('Teams channel is not connected');
    }
    ...
    const info: TeamsConversationInfo = {
      conversationId,
      conversationType:
        (conv.conversationType as TeamsConversationInfo['conversationType']) ?? 'personal',
      tenantId: (conv.tenant as Record<string, unknown>)?.id as string | undefined,
    ...
    req.on('end', async () => {
      try {
        ...
        const message: TeamsMessage = {
          ...
          conversationId:
            ((activity.conversation as Record<string, unknown>)?.id as string) ?? '',
          fromId: ((activity.from as Record<string, unknown>)?.id as string) ?? '',
          fromName: ((activity.from as Record<string, unknown>)?.name as string) ?? undefined,
          text: (activity.text as string) ?? undefined,
          messageType: (activity.type as TeamsMessage['messageType']) ?? 'message',
  ...
  private verifyActivitySignature(_req: http.IncomingMessage, _body: string): boolean {
