...
export class MatrixChannel extends BaseChannel<MatrixConfig, MatrixMessage> {
  ...
  async addReaction(roomId: string, eventId: string, key: string): Promise<string> {
    if (!this.isConnected()) {
      throw new Error('Matrix channel is not connected');
    }
  ...
  async getUserInfo(userId: string): Promise<MatrixUserInfo> {
    ...
    const [profile, presence] = await Promise.all([
      this.request('GET', `/_matrix/client/v3/profile/${encodeURIComponent(userId)}`),
      this.request('GET', `/_matrix/client/v3/presence/${encodeURIComponent(userId)}/status`),
    ]);
    ...
    return {
      ...
      presence: (presenceData.presence as MatrixUserInfo['presence']) ?? 'offline',
    ...
    while (!signal.aborted) {
      try {
        ...
        for (const room of Object.values(roomsData)) {
          ...
          const events = timeline?.events as Array<Record<string, unknown>> | undefined;
          if (!events) continue;
        ...
        this.retryCount++;
        if (this.retryCount > (this.getConfig().maxRetries ?? DEFAULT_MAX_RETRIES)) {
          this.emitError(
        ...
        const delay =
          (this.getConfig().retryDelay ?? DEFAULT_RETRY_DELAY) * this.retryCount;
  ...
  private parseMessageEvent(
    ...
    if (relatesTo) {
      if (relatesTo.rel_type === 'm.thread') {
        ...
        inReplyTo = (relatesTo['m.in_reply_to'] as Record<string, unknown>)?.event_id as
          | string
          | undefined;
  ...
  private async request(
    ...
    return this.makeHttpRequest(url, method, headers, body, options?.timeout ?? 10000);
  }
