...
export class MultiProvider implements LLMProvider {
	...
	constructor(config: FallbackConfig) {
		...
		this.providers = config.providers.map((p) =>
			registry.create(p.provider as ProviderType, p),
		);
	...
	async listModels(): Promise<string[]> {
		...
		for (const provider of this.providers) {
			try {
				...
				if (this.log)
					console.warn(`Failed to list models for ${provider.name}`);
			}
		}
	...
	async chat(request: ChatCompletionRequest): Promise<ChatCompletionResponse> {
		...
		for (let attempt = 0; attempt < this.providers.length; attempt++) {
			...
			try {
				...
				if (this.log) {
					console.warn(
						`[fallback] Provider ${provider.name} failed: ${err.message}`,
					);
				}
			}
		}
		...
		const errorDetails = errors
			...
			.join("; ");
		throw new Error(
			`All providers failed (${errors.length}/${this.providers.length}): ${errorDetails}`,
		);
	}
			...
			try {
				...
				for await (const chunk of provider.chatStream(request)) {
					if (firstChunk) {
						...
						if (this.log) {
							console.log(
								`[fallback] Successfully streaming from: ${provider.name}`,
							);
						}
					}
					yield chunk;
