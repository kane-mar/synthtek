        ...
        .action(async () => {
          try {
            ...
            for (const plugin of plugins) {
              ...
              logger.info(`Plugin: ${plugin} (${stats.isDirectory() ? 'directory' : 'file'})`);
            }
          } catch (err) {
            logger.error('Failed to list plugins', { error: (err as Error).message });
            process.exit(1);
          }
        })
    )
      ...
      new Command('install')
        ...
        .action(async (source: string) => {
          try {
            ...
            logger.error('Failed to install plugin', { error: (err as Error).message });
            process.exit(1);
          }
        })
    )
      ...
      new Command('uninstall')
        ...
        .action(async (name: string) => {
          try {
            ...
            logger.error('Failed to uninstall plugin', { error: (err as Error).message });
            process.exit(1);
          }
        })
    )
      ...
      new Command('info')
        ...
        .action(async (name: string) => {
          try {
            ...
            logger.error('Failed to get plugin info', { error: (err as Error).message });
            process.exit(1);
          }
        })
    );
