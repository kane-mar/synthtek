...
export function registerWebuiCommand(program: Command): void {
  program
    ...
    .option('--host <host>', 'Host to bind to', process.env.WEBUI_HOST || '0.0.0.0')
    .option('--port <port>', 'Port to listen on', process.env.WEBUI_PORT || '8080')
    .action(async (opts: { host?: string; port?: string }) => {
      ...
      const webuiConfig = {
        ...
        sessionTimeout: parseInt(process.env.WEBUI_SESSION_TIMEOUT || '3600', 10),
      };

      const server = new WebUIServer(webuiConfig);
      await server.start();

      logger.info('WebUI started', { host: webuiConfig.host, port: webuiConfig.port });
