  ...
  program
    ...
    .addCommand(
      new Command('start')
        ...
        .action(async (opts: { model?: string; prompt?: string; temperature?: string }) => {
          try {
      ...
      new Command('chat')
        ...
        .action(async (message: string, opts: { model?: string }) => {
          try {
            ...
            await agent.start();
            const result = await agent.processMessageWithCallback({ role: 'user', content: message }, async (_messages) => {
              return { content: 'Agent is running. Configure an LLM provider for real responses.', totalTokens: 0 };
            });
            console.log(result.response);
            await agent.stop();
          } catch (err) {
            logger.error('Agent chat failed', { error: (err as Error).message });
            process.exit(1);
          }
        })
    );
