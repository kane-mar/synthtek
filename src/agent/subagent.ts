...
export class SubagentSpawner {
  ...
  async spawn(
    ...
    this.logger.info('Spawning subagent', { id: id.slice(0, 8), task: config.task.slice(0, 100) });

    // Build subagent config
    const loopConfig: AgentLoopConfig = {
      systemPrompt: config.systemPrompt ?? 'You are a helpful AI assistant working on a specific task.',
    ...
    const hooks: AgentHooks = {
      onBeforeMessage: async (_message) => {
        this.logger.debug('Subagent processing message', { id: id.slice(0, 8) });
      },
      ...
      onBeforeLLMCall: async (messages) => {
        this.logger.debug('Subagent LLM call', { id: id.slice(0, 8), messageCount: messages.length });
      },
      onAfterLLMCall: async (_response, tokens) => {
        this.logger.debug('Subagent LLM response', { id: id.slice(0, 8), tokens });
      },
    };
    ...
    try {
      ...
      const executionPromise = subagentLoop.processMessage(subagentMessage, parentProvider);
