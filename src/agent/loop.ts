...
export class AgentLoop {
  ...
  private createNonStreamingStrategy(
    ...
    return async (messages): Promise<LlmCallResult> => {
      const result = await this.callLLMWithRetry(llmProvider, this.buildLlmRequest(llmProvider, messages));
      return {
  ...
  private async executeAgentLoop(
    ...
    onLlmError: (errorMsg: string, errors: string[], toolCallsMade: number, startTime: number) => AgentLoopResult,
    ...
    while (toolCallsMade < this.config.maxToolCalls) {
      ...
      let nativeToolCalls: Array<{ id: string; name: string; arguments: Record<string, unknown> }> | undefined;
      try {
     ...
     return this.buildAgentLoopResult(responseContent, toolCallsMade, startTime, errors);
   }
   ...
   private async executeToolCalls(
     llmResponse: string,
     nativeToolCalls: Array<{ id: string; name: string; arguments: Record<string, unknown> }> | undefined,
     ...
     const toolCalls = nativeToolCalls && nativeToolCalls.length > 0
       ? nativeToolCalls
  ...
  async *processMessageStream(
    ...
    while (toolCallsMade < this.config.maxToolCalls) {
      ...
      try {
        ...
        for (let attempt = 0; attempt <= maxRetries; attempt++) {
          if (this.isCircuitBreakerOpen()) {
            throw new Error('Circuit breaker is open — LLM calls are temporarily disabled');
          }
  ...
  registerTool(tool: { name: string; description: string; parameters: Record<string, unknown> }, handler: (args: Record<string, unknown>) => Promise<{ content: string; error?: string }>): void {
