...
export class ContextWindowManager {
  ...
  compact(): ContextCompactionResult {
    ...
    const recentCount = Math.max(this.config.minRecentMessages, Math.ceil(messages.length * 0.3));
  ...
  private summarizeMessages(messages: AgentMessage[]): string {
    ...
    if (assistantMessages.length > 0) {
      ...
      parts.push(`Assistant (${assistantMessages.length} messages, showing last 3):`);
      for (const msg of lastFew) {
        const preview = msg.content.length > 200 ? msg.content.slice(0, 200) + '...' : msg.content;
        parts.push(`  - ${preview}`);
      }
