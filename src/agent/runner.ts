...
import { AgentLoop, AgentLoopConfig, AgentHooks } from './index.js';
import { SimpleLogger } from '../core/logger.js';
import { TelegramChannel } from '../channels/telegram/channel.js';
import { TelegramConfig } from '../channels/telegram/types.js';
import type { TelegramMessage } from '../channels/telegram/types.js';
import { getRegistry, registerDefaultProviders, ProviderConfig } from '../providers/index.js';
import { createFallbackProvider, FallbackConfig } from '../providers/fallback.js';
...
export class AgentRunner {
  ...
  constructor(config: AgentRunnerConfig) {
    ...
    const hooks: AgentHooks = {
      ...
      onAfterLLMCall: async (response, tokens) => {
        this.logger.debug('LLM response', { tokens, preview: response.slice(0, 100) });
      },
      ...
      onAfterToolCall: async (result) => {
        if (result.error) {
          this.logger.error(`Tool "${result.name}" failed`, { error: result.error });
        } else {
  ...
  async start(): Promise<void> {
    ...
    if (this.config.telegramToken) {
      ...
      this.logger.info('No Telegram token configured — running in CLI-only mode');
    }
  ...
  async processMessageStream(
    ...
    for await (const chunk of provider.chatStream({
      messages: this.loop.getContext().getFormattedMessages() as import('../providers/types.js').ProviderMessage[],
...
export function createAgentRunnerFromEnv(): AgentRunner {
  const config: AgentRunnerConfig = {
    ...
    logLevel: (process.env.SYNTHTEK_LOG_LEVEL as 'debug' | 'info' | 'warn' | 'error') ?? 'info',
