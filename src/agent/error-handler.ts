...
export class AgentErrorHandler {
  ...
  constructor(config: Partial<AgentLoopConfig>) {
    const retry = config.retry ?? { maxRetries: 3, initialDelay: 1000, maxDelay: 30000, multiplier: 2 };
    ...
    const cb = config.circuitBreaker ?? { failureThreshold: 5, recoveryTimeout: 60000 };
  ...
  classifyError(error: Error, typeHint?: string): ErrorCategory {
    ...
    if (message.includes('context') || message.includes('token limit')) return 'context';
    ...
    if (message.includes('provider') || message.includes('llm') || message.includes('api')) return 'provider';
  ...
  shouldRetry(error: Error, currentAttempt: number): boolean {
    ...
    return matchesPatterns(error.message, this.retryablePatterns) ||
      category === 'provider' ||
      category === 'timeout' ||
      category === 'network' ||
      category === 'rate_limit';
  ...
  isCircuitOpen(): boolean {
    ...
    if (this.circuitBreaker.state === 'open') {
      ...
      if (this.circuitBreaker.lastFailureAt !== null &&
  ...
  formatErrorMessage(category: ErrorCategory, error: Error, attempt: number): string {
    const prefix = this.categoryPrefix(category);
    const retryInfo = attempt > 0 ? ` (attempt ${attempt}/${this.maxRetries})` : '';
