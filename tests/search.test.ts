...
describe('SearchService', () => {
  ...
  before(() => {
    ...
    writeFileSync(join(TEST_DIR, 'file2.txt'), 'another file\nwith different content');
    writeFileSync(join(TEST_DIR, 'file3.md'), '# Markdown\nSome text\nhello here');
  });
  ...
  describe('glob', () => {
    ...
    it('respects headLimit', async () => {
      const result = await service.glob({ pattern: '*', path: TEST_DIR, headLimit: 2 });
    ...
    it('applies offset', async () => {
      const result = await service.glob({ pattern: '*', path: TEST_DIR, offset: 1 });
