...
import { PluginLoggerManager, PluginLogger } from '../../src/logging/plugins.js';
...
test('RotatingFileLogger writes to file', async () => {
  ...
  assert.ok(files[0].startsWith('test-service-'), 'Log file should have correct prefix');
  ...
  assert.ok(content.includes('"level":"info"'), 'Log should be JSON with level');
...
test('RotatingFileLogger respects log level', async () => {
  ...
  assert.ok(!content.includes('should not appear'), 'Debug/info should be filtered');
