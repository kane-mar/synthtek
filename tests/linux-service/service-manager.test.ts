...
describe("SystemdServiceManager", () => {
	...
	describe("generateServiceFile", () => {
		it("generates a valid systemd unit file with minimal config", () => {
			...
			ok(
				content.includes("Description=Synthtek AI Agent Service"),
				"contains description",
			);
			ok(
				content.includes("ExecStart=/usr/bin/node /opt/synthtek/dist/cli.js"),
				"contains ExecStart",
			);
			ok(content.includes("User=synthtek"), "contains user");
			ok(content.includes("Group=synthtek"), "contains group");
			ok(
				content.includes("WorkingDirectory=/opt/synthtek"),
		...
		it("includes EnvironmentFile when configured", () => {
			...
			ok(
				content.includes("EnvironmentFile=/etc/synthtek/env"),
		...
		it("includes StandardOutput=journal when journal forwarding enabled", () => {
			...
			const content = manager.generateServiceFile(config);
			ok(
				content.includes("StandardOutput=journal"),
				"contains StandardOutput=journal",
			);
			ok(
				content.includes("StandardError=journal"),
		...
		it("includes SyslogIdentifier when configured", () => {
			...
			const content = manager.generateServiceFile(config);
			ok(
				content.includes("SyslogIdentifier=synthtek-agent"),
				"contains SyslogIdentifier",
			);
		});
		...
		it("includes custom systemd options", () => {
			...
			const content = manager.generateServiceFile(config);
			ok(
				content.includes("TimeoutStartSec=30"),
				"contains custom TimeoutStartSec",
			);
			ok(
				content.includes("TimeoutStopSec=10"),
				"contains custom TimeoutStopSec",
			);
		...
		it("generates with minimal required fields", () => {
			...
			ok(
				content.includes("Description=minimal-service"),
	...
	describe("log management", () => {
		...
		it("generates log rotation config content", () => {
			...
			const rotationConfig = manager.generateLogRotationConfig(
				"synthtek-agent",
				logConfig,
			);
			...
			ok(
				rotationConfig.content.includes("/var/log/synthtek"),
				"includes log directory",
			);
			ok(rotationConfig.content.includes("rotate 10"), "includes rotate count");
			ok(
				rotationConfig.content.includes("compress"),
				"includes compress directive",
			);
		});
   ...
   Main PID: 12345 (code=exited, status=1/FAILURE)`;
	...
	describe("service name helpers", () => {
		it("appends .service suffix", () => {
			const managerWithName = new SystemdServiceManager({
	...
	describe("service file path", () => {
		it("returns correct systemd path", () => {
			...
			strictEqual(
				managerWithName.serviceFilePath,
		...
		it("uses custom systemd path when provided", () => {
			...
			strictEqual(
				managerWithCustomPath.serviceFilePath,
				"/custom/systemd/path/my-service.service",
			);
		});
	});
	...
	describe("restart policy", () => {
		...
		it("accepts all valid restart policies", () => {
			for (const policy of policies) {
				...
				const content = manager.generateServiceFile(config);
				ok(
					content.includes(`Restart=${policy}`),
					`accepts restart policy: ${policy}`,
				);
			}
		});
	...
	describe("service file structure", () => {
		...
		it("includes documentation link in Unit section", () => {
			...
			ok(
				...
				"contains Documentation directive",
			);
		});
	});
});
