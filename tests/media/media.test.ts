...
describe('MediaProcessor', () => {
  ...
  describe('document handling', () => {
    ...
    it('identifies DOCX document', () => {
      const info = processor.identifyDocument('doc.docx', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document');
      strictEqual(info!.format, 'docx');
      strictEqual(info!.type, 'document');
    });

    it('identifies XLSX document', () => {
      const info = processor.identifyDocument('spreadsheet.xlsx', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
  ...
  describe('upload handling', () => {
    ...
    it('validates upload content type', () => {
      ok(processor.validateUpload('photo.jpg', 'image/jpeg'), 'JPEG upload valid');
      ok(!processor.validateUpload('file.xyz', 'application/octet-stream'), 'XYZ upload invalid');
    });
  });
