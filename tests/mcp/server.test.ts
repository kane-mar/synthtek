...
describe('MCPServer', () => {
  ...
  describe('tools', () => {
    ...
    it('throws error for unknown tool', async () => {
      await server.callTool('nonexistent', {}).catch((err: unknown) => {
        ...
        ok((err as Error).message.includes('not found'), 'error mentions not found');
      });
    });
  });
  ...
  describe('resources', () => {
    ...
    it('registers a resource with URI and description', () => {
      server.registerResource({
        ...
        handler: async () => JSON.stringify({ name: 'synthtek', version: '1.0.0' }),
      });
    ...
    it('lists registered resources', async () => {
      ...
      const configRes = resources.find((r: { uri: string }) => r.uri === 'config://agent');
  ...
  describe('prompts', () => {
    ...
    it('lists registered prompts', async () => {
      ...
      const summarizePrompt = prompts.find((p: { name: string }) => p.name === 'summarize');
