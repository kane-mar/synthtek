...
describe('MCPServer built-in tools', () => {
  ...
  describe('filesystem tools', () => {
    it('registers read_file tool', async () => {
      ...
      const readFileTool = tools.find((t: { name: string }) => t.name === 'read_file');
      ok(readFileTool, 'read_file tool registered');
      ok(readFileTool.description.toLowerCase().includes('read'), 'description mentions read');
    });
    ...
    it('registers write_file tool', async () => {
      ...
      const tools = await server.listTools();
      const writeFileTool = tools.find((t: { name: string }) => t.name === 'write_file');
    ...
    it('write_file tool writes content to a file', async () => {
      ...
      ok(readResult.content.includes('Written by MCP'), 'file contains written content');
    });
  });
