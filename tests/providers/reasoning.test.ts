...
test('OpenAIProvider reasoning model sends reasoning_effort', async () => {
  ...
  try {
    ...
    assert.equal((capturedBody as Record<string, unknown>).reasoning_effort, 'medium');
    assert.equal((capturedBody as Record<string, unknown>).temperature, undefined);
  } finally {
...
test('OpenAIProvider non-reasoning model does NOT send reasoning_effort', async () => {
  ...
  try {
    ...
    assert.ok(capturedBody);
    assert.equal((capturedBody as Record<string, unknown>).reasoning_effort, undefined);
  } finally {
    global.fetch = originalFetch;
  }
});
  ...
  global.fetch = async () =>
    ({
      ...
      json: async () => ({
        ...
        model: 'claude-3-5-sonnet-20241022',
        content: [
          { type: 'thinking', thinking: 'Step 1: Analyze the question. Step 2: Consider options...' },
...
test('AnthropicProvider without extended thinking does NOT send thinking budget', async () => {
  ...
  try {
    ...
    assert.equal((capturedBody as Record<string, unknown>).budget_tokens, undefined);
  } finally {
    global.fetch = originalFetch;
  }
});
