...
function createMockChannel(id: string, name: string, type: number, topic?: string) {
  ...
  const mockCh = {
    ...
    sendTyping: async () => { lastTyping = true; },
    edit: async (opts: { content: string }) => { lastEdited = opts.content; },
    delete: async () => { lastDeleted = true; },
    react: async () => { lastReacted = true; },
    reactions: {
      cache: { get: () => null },
    },
    messages: {
...
describe('DiscordChannel', () => {
  ...
  describe('sendTyping', () => {
    it('throws when channel not found', async () => {
      ...
      await rejects(
        channel.sendTyping('nonexistent'),
        /Channel.*not found/,
      );
    });
  });
  ...
  describe('hasPermission', () => {
    it('returns false when channel not found', async () => {
      channel = new DiscordChannel({ token: 'test-token' });
      const hasPerm = await channel.hasPermission('nonexistent', 'administrator');
      equal(hasPerm, false);
    });
  });
