...
describe('SlackChannel', () => {
  ...
  describe('reactions', () => {
    ...
    it('throws when removing reaction with invalid token', async () => {
      await rejects(
        channel.removeReaction('C123456', '1234567890.123456', 'white_check_mark'),
        /Slack API error/,
      );
    });
  ...
  describe('channel info', () => {
    ...
    it('throws when getting channel info with invalid token', async () => {
      await rejects(
        channel.getChannelInfo('C123456'),
        /Slack API error/,
      );
    });
  ...
  describe('message processing', () => {
    ...
    it('handles multiple message handlers', async () => {
      ...
      channel.onMessage(async (msg) => { handler1.push(msg); });
      channel.onMessage(async (msg) => { handler2.push(msg); });
