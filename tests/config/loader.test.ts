...
function createTempDir(): string {
	const dir = join(
		tmpdir(),
		`synthtek-test-${Date.now()}-${Math.random().toString(36).slice(2)}`,
	);
...
describe("ConfigLoader", () => {
	...
	test("environment variables override file config", () => {
		...
		if (origProvider === undefined) delete process.env.SYNTHTEK_PROVIDER;
		else process.env.SYNTHTEK_PROVIDER = origProvider;
		if (origApiKey === undefined) delete process.env.SYNTHTEK_API_KEY;
		else process.env.SYNTHTEK_API_KEY = origApiKey;
		if (origLogLevel === undefined) delete process.env.SYNTHTEK_LOG_LEVEL;
		else process.env.SYNTHTEK_LOG_LEVEL = origLogLevel;
	...
	test("loads loop config from file", () => {
		...
		assert.equal(
			config.loopConfig.systemPrompt,
	...
	test("supports all config file name variants", () => {
		const variants = [".synthtek-config.json", "synthtek.config.json"];
