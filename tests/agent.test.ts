...
import { equal, ok, deepStrictEqual, throws } from 'node:assert';
import { AgentLoop, ContextWindowManager, ToolRegistry, HeartbeatManager } from '../src/agent/index.js';
import type { AgentMessage, ToolCall, AgentLoopResult } from '../src/agent/types.js';
...
function createMockLLM(response: { content: string; tokens?: number }, shouldThrow?: boolean): LLMProvider {
  if (shouldThrow) {
    return {
      name: 'mock',
      chat: async () => { throw new Error('API timeout'); },
  ...
  return {
    name: 'mock',
    chat: async () => ({ ...response, model: 'mock', totalTokens: response.tokens }),
    chatStream: async function* () { yield { delta: '', done: true }; },
    listModels: async () => [],
    healthCheck: async () => true,
    getConfig: () => ({ provider: 'mock', apiKey: '' }),
...
describe('AgentLoop', () => {
  ...
  describe('processMessage — plain text response', () => {
    it('returns the LLM response when no tool calls are needed', async () => {
      loop = new AgentLoop();
      const mockLLM = createMockLLM({ content: 'Hello! How can I help you?', tokens: 100 });
    ...
    it('executes tool calls and loops back to LLM', async () => {
      ...
      const mockLLM: LLMProvider = {
        ...
        chat: async () => {
          ...
          return { content: 'Here is the information you requested.', model: 'mock', totalTokens: 100 };
        },
        chatStream: async function* () { yield { delta: '', done: true }; },
        listModels: async () => [],
        healthCheck: async () => true,
    ...
    it('handles tool errors', async () => {
      ...
      const mockLLM: LLMProvider = {
        ...
        chat: async () => {
          ...
          return { content: 'I could not find the file.', model: 'mock', totalTokens: 80 };
        },
        chatStream: async function* () { yield { delta: '', done: true }; },
        listModels: async () => [],
        healthCheck: async () => true,
...
describe('ToolRegistry', () => {
  ...
  describe('register', () => {
    it('registers a tool with its handler', () => {
      ...
      const handler = async () => ({ callId: '', name: 'test_tool', content: 'result' });
    ...
    it('throws on duplicate registration', () => {
      ...
      registry.register(tool, async () => ({ callId: '', name: 'dup', content: 'a' }));
      throws(
        () => registry.register(tool, async () => ({ callId: '', name: 'dup', content: 'b' })),
        /already registered/,
      );
    });
  });
  ...
  describe('unregister', () => {
    it('removes a tool', () => {
      ...
      registry.register(tool, async () => ({ callId: '', name: 'test_tool', content: 'result' }));
  ...
  describe('execute', () => {
    ...
    it('returns error for disabled tool', async () => {
      ...
      registry.register(tool, async () => ({ callId: '', name: 'disabled_tool', content: 'should not run' }));
  ...
  describe('executeAll', () => {
    it('executes multiple tools', async () => {
      ...
      registry.register(tool1, async () => ({ callId: '', name: 'tool1', content: 'r1' }));
      registry.register(tool2, async () => ({ callId: '', name: 'tool2', content: 'r2' }));
  ...
  describe('getTools', () => {
    it('returns only enabled tools', () => {
      ...
      registry.register(enabled, async () => ({ callId: '', name: 'enabled', content: 'a' }));
      registry.register(disabled, async () => ({ callId: '', name: 'disabled', content: 'b' }));
