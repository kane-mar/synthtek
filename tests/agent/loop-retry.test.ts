...
test('AgentLoop retries on LLM failure by default', async () => {
  ...
  const { provider, chatCalls } = createMockProvider({ chatResponses: responses });
...
test('AgentLoop does not retry non-retryable errors', async () => {
  const responses = [
    { content: '', error: new Error('Invalid API key') },
  ];
...
test('AgentLoop handles circuit breaker blocking requests', async () => {
  ...
  const successProvider = {
    name: 'mock',
    chat: async () => ({ content: 'Should not reach here', model: 'mock', tokens: 10 }),
    chatStream: async function* () {
      yield { delta: '', done: true };
    },
    listModels: async () => [],
