...
describe("Memory Skills", () => {
	...
	describe("MemoryDefragSkill", () => {
		...
		it("detects and removes duplicates", async () => {
			...
			await memory.create({
				content: "Duplicate content for testing purposes here",
			});
			await memory.create({
				content: "Duplicate content for testing purposes here",
			});
	...
	describe("MemoryNotesSkill", () => {
		...
		it("creates and retrieves notes", async () => {
			...
			const note = await skill.createNote(
				"Test Note",
		...
		it("updates notes", async () => {
			...
			const note = await skill.createNote("Update Test", "Original content", [
				"test",
			]);
			const updated = await skill.updateNote(note.id, {
				title: "Updated Title",
				pinned: true,
			});
		...
		it("lists and searches notes", async () => {
			...
			await skill.createNote("Searchable Note", "Find me in search", [
				"searchable",
			]);
		...
		it("deletes notes", async () => {
			...
			const note = await skill.createNote("To Delete", "Will be deleted", [
				"test",
			]);
	...
	describe("MemoryTasksSkill", () => {
		...
		it("creates and retrieves tasks", async () => {
			...
			const task = await skill.createTask(
				"Test Task",
		...
		it("detects overdue tasks", async () => {
			...
			await skill.createTask("Overdue Task", "Was due yesterday", {
				dueDate: pastDate,
			});
	...
	describe("MemoryIngestSkill", () => {
		...
		it("ingests text content", async () => {
			...
			const result = await skill.ingestText(
				"test-source",
		...
		it("auto-tags content based on analysis", async () => {
			...
			const result = await skill.ingestText(
				"auto-tag-test",
				"This is a code function with import and export statements",
				{
					tags: ["code-test"],
				},
			);
