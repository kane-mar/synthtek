...
describe('EntityExtractorImpl', () => {
  ...
  describe('extractEntities', () => {
    it('should extract person entities', () => {
      const entities = extractor.extractEntities('John Smith is a software engineer.');
    ...
    it('should extract organization entities', () => {
      const entities = extractor.extractEntities('Google announced a new product.');
    ...
    it('should extract location entities', () => {
      const entities = extractor.extractEntities('The meeting is in San Francisco.');
    ...
    it('should extract date entities', () => {
      const entities = extractor.extractEntities('The deadline is December 31, 2026.');
      const date = entities.find((e) => e.type === 'Date');
      assert.ok(date, 'Should extract Date entity');
    });

    it('should extract event entities', () => {
      const entities = extractor.extractEntities('The conference starts next week.');
    ...
    it('should return empty array for text without entities', () => {
      const entities = extractor.extractEntities('This is just some random text.');
      assert.ok(entities.length === 0 || entities.every((e) => e.type === 'Concept'));
    });

    it('should handle multiple entities in one text', () => {
      const entities = extractor.extractEntities('John works at Google in San Francisco.');
