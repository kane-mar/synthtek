...
describe('MediaPreviewComponent', () => {
  ...
  describe('media filtering', () => {
    it('filters by type', () => {
      preview.registerMedia({ filename: 'photo.jpg', mimeType: 'image/jpeg', size: 100, url: 'http://x' });
      preview.registerMedia({ filename: 'rec.mp3', mimeType: 'audio/mpeg', size: 200, url: 'http://x' });
      const images = preview.filterByType('image');
      strictEqual(images.length, 1);
    });

    it('filters by size range', () => {
      preview.registerMedia({ filename: 'small.jpg', mimeType: 'image/jpeg', size: 1000, url: 'http://x' });
      preview.registerMedia({ filename: 'large.jpg', mimeType: 'image/jpeg', size: 1000000, url: 'http://x' });
      const small = preview.filterBySize(0, 5000);
      strictEqual(small.length, 1);
    });

    it('returns all when no filter', () => {
      preview.registerMedia({ filename: 'a.jpg', mimeType: 'image/jpeg', size: 100, url: 'http://x' });
      preview.registerMedia({ filename: 'b.mp3', mimeType: 'audio/mpeg', size: 200, url: 'http://x' });
  ...
  describe('render', () => {
    it('renders media preview HTML', () => {
      preview.registerMedia({ filename: 'photo.jpg', mimeType: 'image/jpeg', size: 102400, url: 'http://localhost/media/photo.jpg' });
    ...
    it('renders audio preview', () => {
      preview.registerMedia({ filename: 'rec.mp3', mimeType: 'audio/mpeg', size: 512000, url: 'http://localhost/media/rec.mp3' });
      const html = preview.render();
      ok(html.includes('media-audio'), 'includes audio class');
    });

    it('renders video preview', () => {
      preview.registerMedia({ filename: 'clip.mp4', mimeType: 'video/mp4', size: 10240000, url: 'http://localhost/media/clip.mp4' });
      const html = preview.render();
      ok(html.includes('media-video'), 'includes video class');
    });

    it('renders document preview', () => {
      preview.registerMedia({ filename: 'report.pdf', mimeType: 'application/pdf', size: 204800, url: 'http://localhost/media/report.pdf' });
      const html = preview.render();
  ...
  describe('media stats', () => {
    ...
    it('returns total media size', () => {
      preview.registerMedia({ filename: 'a.jpg', mimeType: 'image/jpeg', size: 1000, url: 'http://x' });
      preview.registerMedia({ filename: 'b.mp3', mimeType: 'audio/mpeg', size: 2000, url: 'http://x' });
      strictEqual(preview.getTotalSize(), 3000);
    });

    it('returns formatted total size', () => {
      preview.registerMedia({ filename: 'a.jpg', mimeType: 'image/jpeg', size: 1024, url: 'http://x' });
