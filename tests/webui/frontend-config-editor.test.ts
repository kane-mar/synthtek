...
import type { ConfigField, ConfigSection } from '../../src/webui/frontend/types.js';
...
describe('ConfigEditorComponent', () => {
  ...
  describe('value management', () => {
    it('gets field value', () => {
      editor.addSection({ name: 'general', fields: [] });
      editor.addField('general', { key: 'host', label: 'Host', type: 'text', value: 'localhost' });
  ...
  describe('validation', () => {
    it('validates required text fields', () => {
      editor.addSection({ name: 'general', fields: [] });
      editor.addField('general', { key: 'host', label: 'Host', type: 'text', value: '', required: true });
      ok(!editor.validateSection('general'), 'empty required field fails');
    });

    it('validates non-empty required fields', () => {
      editor.addSection({ name: 'general', fields: [] });
      editor.addField('general', { key: 'host', label: 'Host', type: 'text', value: 'localhost', required: true });
      ok(editor.validateSection('general'), 'non-empty required field passes');
    });

    it('validates number range', () => {
      editor.addSection({ name: 'general', fields: [] });
      editor.addField('general', { key: 'port', label: 'Port', type: 'number', value: 3000 });
  ...
  describe('export/import', () => {
    ...
    it('imports config from JSON', () => {
      const json = JSON.stringify([{ name: 'general', fields: [{ key: 'host', label: 'Host', type: 'text', value: 'localhost' }] }]);
