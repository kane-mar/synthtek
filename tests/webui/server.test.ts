...
describe("WebUIServer", () => {
	...
	it("frontend meets accessibility guidelines", async () => {
		...
		try {
			...
			assert.ok(
				...
				"chat container should have aria-live",
			);
			assert.ok(
				html.includes('role="log"'),
				"chat container should have role=log",
			);

			// C2: focus-visible states in CSS
			assert.ok(
				html.includes("focus-visible"),
				"should define :focus-visible styles",
			);
			...
			assert.ok(
				html.includes('<label for="'),
				"form labels should use for attribute",
			);

			// I3: name and autocomplete on form inputs
			assert.ok(
				html.includes('name="chat-message"'),
				"chat input should have name attribute",
			);
			assert.ok(
				html.includes('autocomplete="off"'),
				"chat input should disable autocomplete",
			);

			// I4: prefers-reduced-motion media query
			assert.ok(
				html.includes("prefers-reduced-motion"),
				"should honor reduced motion preference",
			);

			// I5: proper ellipsis character
			assert.ok(
				html.includes("Connecting…"),
			...
			assert.ok(
				...
				"buttons should have touch-action",
			);
			...
			assert.ok(
				...
				"should sync URL hash with page state",
			);

			// C4: inline error messages (no alert())
			assert.ok(
				html.includes("modal-error"),
				"should have inline error container",
			);
			assert.ok(
			...
			assert.ok(
				html.includes('id="chat-input-bar"'),
				"chat input bar should have correct ID",
			);
		} finally {
			await server.stop();
		}
	...
	it("adds and retrieves messages", async () => {
		...
		try {
			const sessRes = await fetch(
				`http://${cfg.host}:${cfg.port}/api/sessions`,
				{
					method: "POST",
					headers: { "Content-Type": "application/json" },
					body: JSON.stringify({ userId: "test-user" }),
			...
			const msgRes = await fetch(
				`http://${cfg.host}:${cfg.port}/api/messages`,
				{
					method: "POST",
					headers: { "Content-Type": "application/json" },
					body: JSON.stringify({
			...
			const getRes = await fetch(
				`http://${cfg.host}:${cfg.port}/api/messages?sessionId=${session.id}`,
			);
	...
	it("gets a provider by ID via GET /api/providers/:id", async () => {
		...
		try {
			...
			const createRes = await fetch(
				`http://${cfg.host}:${cfg.port}/api/providers`,
				{
					method: "POST",
					headers: { "Content-Type": "application/json" },
					body: JSON.stringify({
			...
			// Fetch by ID
			const getRes = await fetch(
				`http://${cfg.host}:${cfg.port}/api/providers/${p.id}`,
			);
			assert.equal(getRes.status, 200);
	...
	it("returns 404 for non-existent provider ID", async () => {
		...
		try {
			const res = await fetch(
				`http://${cfg.host}:${cfg.port}/api/providers/nonexistent`,
			);
			assert.equal(res.status, 404);
		} finally {
			await server.stop();
		...
		try {
			...
			const updateRes = await fetch(
				`http://${cfg.host}:${cfg.port}/api/providers/${p.id}`,
				{
					method: "PUT",
					headers: { "Content-Type": "application/json" },
					body: JSON.stringify({ name: "After Update", temperature: 0.5 }),
				},
			);
	...
	it("returns provider presets", async () => {
		...
		try {
			const res = await fetch(
				`http://${cfg.host}:${cfg.port}/api/providers/presets`,
			);
			assert.equal(res.status, 200);
