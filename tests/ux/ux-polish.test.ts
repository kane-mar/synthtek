...
describe('UXPolish', () => {
  ...
  describe('model autocomplete', () => {
    ...
    it('filters suggestions by query', () => {
      ...
      for (const s of suggestions) {
        ok(s.name.toLowerCase().includes('gpt-4'), `suggestion matches query: ${s.name}`);
      }
    });
  ...
  describe('human-readable cron schedules', () => {
    it('formats daily schedule', () => {
      ...
      ok(readable.toLowerCase().includes('daily') || readable.includes('9'), 'mentions daily or 9am');
    });

    it('formats weekly schedule', () => {
      const readable = ux.formatCronSchedule('0 9 * * 1');
      ok(typeof readable === 'string', 'returns string');
      ok(readable.toLowerCase().includes('monday') || readable.toLowerCase().includes('weekly'), 'mentions monday or weekly');
    });

    it('formats monthly schedule', () => {
      const readable = ux.formatCronSchedule('0 0 1 * *');
      ok(typeof readable === 'string', 'returns string');
      ok(readable.toLowerCase().includes('monthly') || readable.includes('1st'), 'mentions monthly or 1st');
    });

    it('handles invalid cron expression', () => {
      const readable = ux.formatCronSchedule('invalid');
      ok(typeof readable === 'string', 'returns string');
      ok(readable.toLowerCase().includes('invalid') || readable.toLowerCase().includes('unknown'), 'indicates invalid');
    });
  });
  ...
  describe('typing indicators', () => {
    ...
    it('respects typing indicator setting', () => {
      const noIndicator = new UXPolish({ ...defaultConfig, showTypingIndicator: false });
  ...
  describe('error messages with actionable suggestions', () => {
    it('provides suggestion for API key error', () => {
      ...
      ok(message.toLowerCase().includes('api') || message.toLowerCase().includes('key'), 'mentions API key');
    });

    it('provides suggestion for connection error', () => {
      const message = ux.formatError('connection_failed');
      ok(typeof message === 'string', 'returns string');
      ok(message.toLowerCase().includes('connection') || message.toLowerCase().includes('network'), 'mentions connection');
    });
