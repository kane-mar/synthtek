...
describe('ApiKeyEncryption', () => {
  ...
  describe('encrypt', () => {
    ...
    it('should return different ciphertext for same input', async () => {
      ...
      assert.notStrictEqual(r1.value, r2.value, 'Ciphertext should differ due to random IV/salt');
    });
  ...
  describe('encryptMany / decryptMany', () => {
    it('should encrypt and decrypt multiple keys', async () => {
      ...
      const encryptedValues = encrypted.map((e) => e.value!).filter(Boolean) as string[];
