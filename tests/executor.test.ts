...
describe('AsyncExecutor', () => {
  it('executes a simple command successfully', async () => {
    ...
    const result = await executor.execute({ command: 'echo hello', shell: true });
  ...
  it('captures stderr on failure', async () => {
    const executor = new AsyncExecutor();
    const result = await executor.execute({ command: 'ls /nonexistent/path', shell: true });
  ...
  it('returns error message on failure', async () => {
    const executor = new AsyncExecutor();
    const result = await executor.execute({ command: 'nonexistent_command_xyz' });
