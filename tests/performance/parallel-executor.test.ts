...
import type { ParallelExecutorConfig, ToolTask } from '../../src/performance/types.js';
...
describe('ParallelExecutor', () => {
  ...
  describe('execute', () => {
    it('executes tasks in parallel', async () => {
      ...
      ok(results.every((r) => r.success), 'all tasks succeeded');
    });
    ...
    it('respects concurrency limit', async () => {
      ...
      ok(maxConcurrent <= defaultConfig.maxConcurrency, `max concurrency ${maxConcurrent} within limit ${defaultConfig.maxConcurrency}`);
    });
    ...
    it('tracks execution duration', async () => {
      ...
      const results = await executor.execute(tasks);
      ok(results[0].duration >= 100, `duration ${results[0].duration}ms >= 100ms`);
    });
  });
  ...
  describe('failFast', () => {
    it('stops execution on first failure when failFast enabled', async () => {
      const failFastExecutor = new ParallelExecutor({ ...defaultConfig, failFast: true });
      const tasks: ToolTask[] = [
        {
          id: 'bad',
          name: 'bad',
