...
describe("ContextManager", () => {
	...
	describe("compaction", () => {
		it("compacts when tokens exceed threshold", () => {
			...
			ok(
				before.totalTokens > defaultConfig.compactionThreshold,
			...
			ok(
				...
				"tokens reduced after compaction",
			);
			ok(
				after.messageCount >= defaultConfig.minKeptMessages,
				"min messages kept",
			);
		...
		it("preserves recent messages", () => {
			...
			const snapshot = manager.snapshot();
			const hasRecent = snapshot.messages.some(
				(m) => m.content === recentContent,
			);
