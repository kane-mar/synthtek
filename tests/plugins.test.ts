...
import type { Plugin, PluginConfig, Logger } from '../src/plugins/types.js';
...
describe('PluginLoader', () => {
  it('should load a valid plugin', async () => {
    ...
    const config = createMockConfig(join(process.cwd(), 'test-plugins'));
    const loader = new PluginLoader({ config, globalLogger: logger, globalEvents: events });
  ...
  it('should reject invalid config types', async () => {
    ...
    const plugins = await loader.load(discovered);
    assert.strictEqual(plugins.length, 0, 'Plugin with invalid config should not be loaded');
  });
});
...
describe('PluginManager', () => {
  it('should discover, load, init, run, and teardown plugins', async () => {
    ...
    const manager = new PluginManager({ config, globalLogger: logger, globalEvents: events });
